...
#  그대로 유지되어 Retrofit 경로 문자열(@PUT("rider/location") 등)로 통신 흐름을
#  추적할 수 있다. (배달앱과 동일한 "난독화 적용, 흐름 노출" 상태)
#
#  주의: R8은 식별자 치환일 뿐 문자열 상수는 난독화하지 않는다. 문자열 암호화·
#  제어흐름 난독화는 별도 상용 도구 영역이다. 라이더 앱은 네이티브(.so)가 없다.
# ==========================================================================

# Gson 리플렉션이 필드명으로 JSON을 매핑하므로 DTO 모델은 필드까지 보존한다.
# (모델을 난독화하면 서버 JSON 키와 어긋나 파싱이 깨진다.)
-keep class com.hackmin.connect.data.model.** { *; }
...
# BuildConfig(로깅 스위치·PayloadCrypto 시크릿 참조).
-keep class com.hackmin.connect.BuildConfig { *; }
