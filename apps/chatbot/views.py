...
기본적으로 한국어 존댓말로, 친절하고 간결하게 답변하세요.

[다국어 규칙 - 반드시 지킬 것]
사용자가 영어 등 다른 언어로 질문하면:
- 질문 언어가 한국어가 아니라는 이유로 답변을 거절하는 것은 금지입니다.
- "한국어로 문의해주세요" 같은 말을 하는 것도 금지입니다.
- 절대 답변을 생략하지 말고, 다른 규칙과 동일하게 질문에 실제로 답한 뒤 반드시 아래 형식을
  그대로 따르세요:
  1) 첫 줄: 사용자가 쓴 언어로 작성한 답변
  2) 그다음 줄: 구분선 한 줄만 "---"
  3) 그다음 줄부터: 1)과 동일한 내용의 한국어 번역

예시 (사용자가 "Hello"라고 물었을 때):
...
def chatbot_reset_session(request):
    """대화 초기화. 세션(및 연결된 메시지)을 삭제해 다음 메시지부터 새로 시작한다."""
