...
<#
.SYNOPSIS
    릴리즈 APK를 빌드 → 서명 → LDPlayer에 설치(옵션: 자동 실행)까지 한 번에 처리한다.

.DESCRIPTION
    deliver_app 의 release 빌드는 signingConfig 가 없어 assembleRelease 결과물이
...
.PARAMETER GuardMode
    네이티브 가드 종료 방식. inline(기본, 조용히 종료) 또는 message(토스트 후 종료).
    ./gradlew assembleRelease -PguardMode=... 로 전달된다.

.PARAMETER Port
    LDPlayer 인스턴스의 adb 포트. 1번 인스턴스는 보통 5555, 이후 5557, 5559 ...
    (LDPlayer 설정 → 기타 설정 → "로컬 연결(ADB 디버깅)"을 켜 두어야 한다.)
...
.EXAMPLE
    powershell -ExecutionPolicy Bypass -File deliver_app\deploy_ldplayer.ps1 -Launch
.EXAMPLE
    .\deploy_ldplayer.ps1 -GuardMode message -Port 5555 -Launch
#>
[CmdletBinding()]
param(
    [ValidateSet('inline', 'message')]
    [string]$GuardMode = 'inline',
    [int]$Port = 5555,
    [string]$Serial,
    [switch]$Launch,
    [switch]$SkipBuild
...
$AppId = 'com.hackmin.app'
$LauncherComponent = "$AppId/.ui.auth.LoginActivity"
$AppDir = $PSScriptRoot                       # ...\deliver_app
...
function Resolve-Jdk21 {
    ...
    foreach ($c in $candidates) {
        ...
        if ($hit -and (Test-Path (Join-Path $hit.FullName 'bin\java.exe'))) {
            # java.exe 실행(-version)은 stderr 로 출력 → PS 5.1 에서 NativeCommandError 를
            # 유발하므로, 대신 JDK 홈의 'release' 파일에서 버전을 읽는다.
            ...
            if (Test-Path $releaseFile) {
                ...
                # release 파일이 없으면(드묾) 후보 경로를 신뢰해 그대로 사용.
                return $hit.FullName
            }
        }
    }
...
# ── 4) 릴리즈 빌드 (Java 21 강제) ─────────────────────────────────────────
if (-not $SkipBuild) {
    Info "릴리즈 빌드 시작 (guardMode=$GuardMode) ..."
    $env:JAVA_HOME = $jdk
    $gradlew = Join-Path $AppDir 'gradlew.bat'

    # Gradle 9 는 toolchain 자동탐지로 VS Code 번들 JRE(jlink 없음)를 골라
    # JdkImageTransform 을 깨뜨린다. 자동탐지를 끄고 JBR(=JAVA_HOME/org.gradle.java.home)만
    # 쓰게 강제한다. 기존(잘못된 JVM) 데몬도 먼저 정지한다.
    & $gradlew --stop | Out-Null
    & $gradlew -p $AppDir 'assembleRelease' "-PguardMode=$GuardMode" `
...
if (Test-Path $unsigned) {
    ...
    # 이미 서명된 산출물(향후 signingConfig 추가 시)이면 그대로 사용
    $srcApk = $prebuilt
} else {
